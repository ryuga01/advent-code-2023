...
x = x.split('\n')
container = []
wordsx =  ['one', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight', 'nine']
numberx= ['1', '2', '3', '4', '5', '6', '7', '8', '9']
def find_first_num(digit):
    if (not(digit.isdigit())):
        return str(wordsx.index(digit) +1)
    return digit
...
def get_order(number):
    return number.get('order')
def get_number(number):
    return (number['number'])
def index_finder(word, words):
    holder =[]
    text = 'Allowed Hello Hollow'
    for m in re.finditer(word, words):
        holder.append({"order": m.start(), "number": word})
    return holder
def word_to_num(number):
    words = wordsx + numberx
    holder = []
    number_word = number
    for word in words:
        holder +=(index_finder(word, number))
        # holder.append({"order": index_finder(word, numberx), "number": word})
    holder.sort(key=get_order)
    x= list(map(get_number,holder))
    print(x)
    y= str(find_first_num(x[0]))+ str(find_first_num(x[-1]))
    return int(y)

for i in x: 
    container.append(word_to_num(i))
print((container))
print(sum(container))
